...
21. **Visual identity rebuilt (Aug 2026)** — the previous cream/terracotta + Fraunces palette was the single most recognisable AI-generated design default. Replaced with a palette derived from Paulo's own portrait (`cv.png`): cobalt `#2A4BA0`, ochre `#E0A92E`, near-white `#FAFAF8`, ink `#12141A`. Type is **Bricolage Grotesque** (display) / **Archivo** (body) / **JetBrains Mono** (utility). Motion was consolidated to a single orchestrated hero sequence; the blanket scroll-reveal, radial glow blobs and gradient fills were removed.
...
- Match the existing cobalt/ochre visual system (see the `:root` block at the top of any page — `--paper`, `--ink`, `--accent`, `--ochre`, `--line`, `--surface`). The palette is derived from Paulo's portrait, so it must stay in step with that photo. Type stack is `Bricolage Grotesque` (headings), `Archivo` (body), `JetBrains Mono` (codes/labels). All eight pages carry their own copy of the token block — change them together.
